...
public class OCIBundleCommand implements Runnable {
    ...
    public void run() {
        ...
        grabOCIBundle(configuration, path);
            ...
            if (manifest != null) {
                ...
                if (!taskLayers.isEmpty()) {
                    taskLayers.forEach(layer -> {
                        ...
                        extractAndProcessBlob(Paths.get(path, "/tekton"),new File(path + "/blobs/sha256" + "/" + layerDigest.substring(7, layerDigest.length())));
                    });
                }
            }
    ...
    private static void extractAndProcessBlob(Path outPutPath, File blobFile) {
