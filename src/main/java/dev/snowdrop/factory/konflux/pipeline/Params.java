...
public class Params {
   ...
   public static List<Param> KONFLUX_PIPELINERUN_PARAMS() {
      ...
      // NOT USED - params.add(new ParamBuilder().withName("dockerfile").withValue(new ParamValue("src/main/docker/Dockerfile.jvm")).build());
      // NOT USED - params.add(new ParamBuilder().withName("path-context").withValue(new ParamValue(".")).build());
   ...
   public static List<ParamSpec> KONFLUX_PIPELINESPEC_PARAMS() {
      ...
      // Let's make a test without declaring the following params and see what it happens
      // Result: Tekton can parse the generated PipelineRun at the condition that
      //  add some parameters when defined here with default value and used by a task !!
      //  Example:
